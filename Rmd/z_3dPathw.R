...
newse$aa_clustes <- factor(
  c(as.character(aa_clusters), rep("Archetypes", num_archetypes)),
...
tsne_3d$aa_clusters <- newse$aa_clustes
...
plot_3d <- plot_ly(tsne_3d, x = ~X1, y = ~X2, z = ~X3, color = ~Label, colors = c(labels_colors, archetype_color)) %>%
  add_markers(size = ~ ifelse(Label == "Archetype", 8, 3), text = ~ ifelse(Label == "Archetype", seq(1, num_archetypes), "")) %>%
  layout(
