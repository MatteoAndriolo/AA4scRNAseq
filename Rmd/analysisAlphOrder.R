...
folder_path <- Sys.getenv("FOLDER_PATH")
input_file <- Sys.getenv("INPUT_FILE")
output_file <- Sys.getenv("OUTPUT_FILE")

# Check if environment variables are provided
if (folder_path == "" || input_file == "" || output_file == "") {
  stop("FOLDER_PATH, INPUT_FILE, and OUTPUT_FILE environment variables must be provided.")
}

# Construct full paths for input and output files
input_file_path <- file.path(folder_path, input_file)
output_file_path <- file.path(folder_path, output_file)

# Load the object
message("LOG: loading")
obj <- readRDS(input_file_path)
message("LOG: loaded")
classname <- "Melanoma"
class(obj) <- classname
